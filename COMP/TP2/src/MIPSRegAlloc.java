...
public class MIPSRegAlloc implements MIPSCGConstants {
	private static final int R_eq_R_op_R = 0, S_eq_R_op_R = 1, R_eq_S_op_R = 2,
			S_eq_S_op_R = 3, R_eq_R_op_S = 4, S_eq_R_op_S = 5, R_eq_S_op_S = 6,
			S_eq_S_op_S = 7;
	...
	public void loadRegisterMIPS(int r, Operand3a op) {
		...
		if (op.isConstInteger()) {
			...
			if (op.isVarInteger()) {
				out.println(R_INDENT + "lw  $" + r + "," + op.getOffset()
						+ "($" + R_STACK + ")");
				...
				if (op.isLabel()) {
					...
					out.println(R_INDENT + "lui   $" + r + ",%hi("
							+ op.getName3a() + ")");
					out.println(R_INDENT + "addiu   $" + r + ",$" + r + ",%lo("
							+ op.getName3a() + ")");
				} else {
	 ...
	 */
	public MIPSCGTriplet<MIPSRegDesc> allocateRegisters(Operand3a dest,
			Operand3a op1, Operand3a op2) {
		...
		MIPSRegDesc destReg = this.isInRegister(dest);
		MIPSRegDesc op1Reg = this.isInRegister(op1);
		MIPSRegDesc op2Reg = this.isInRegister(op2);
		...
		case S_eq_R_op_R:
			...
			destReg = this.allocateOneRegister(op1Reg.getRegNumber(),
					op2Reg.getRegNumber());
			break;
		case R_eq_R_op_S:
			op2Reg = destReg;
		...
		return new MIPSCGTriplet<MIPSRegDesc>(destReg, op1Reg, op2Reg);
	}
