...
int main(int argc, char ** argv)
{
    ...
    fprintf(stderr, "Before recording start timer\n");

    // Start timer
    CUDA_SAFE_CALL(cudaEventRecord(start, 0));

    fprintf(stderr, "Before launching the kernel\n");

    // Launch kernel
    summation_kernel<<<blocks_in_grid, threads_per_block>>>(data_size, data_out_gpu);

    fprintf(stderr, "Before recording stop event\n");

    // Stop timer
    CUDA_SAFE_CALL(cudaEventRecord(stop, 0));
    CUDA_SAFE_CALL(cudaEventSynchronize(stop));

    fprintf(stderr, "Before getting the results back\n");
    ...
    for (i = 0; i < results_size; ++i) {
        if((i*units_per_thread)%2==0) sum += data_out_cpu[i];
        else sum -= data_out_cpu[i];
    }
