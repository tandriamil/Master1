 ...
 */
int random_int() {
	fprintf(stderr, "Random int generated = %d\n", (int)(rand()%(RAND_MAX_VALUE - RAND_MIN_VALUE) + RAND_MIN_VALUE));
...
int main() {
	...
	// Correct execution
		...
		// Correct execution
			...
			fprintf(stderr, "File correctly created and initialised with fd = %d\n", file);

			// We create a mapped file
			int *mapped_file = (int *)mmap(0, ROW_SIZE * ROW_SIZE * sizeof(int), PROT_READ|PROT_WRITE, MAP_SHARED, file, 0);
