...
public class MyIhm implements IhmInterface {
	...
	private static final String UNDO_CMD = "z";
	private static final String REDO_CMD = "y";
	...
	public MyIhm(InputStream inputStream, MoteurEditeurInterface m) {
		...
		this.commandes.put(UNDO_CMD, new Defaire(this.moteur, this, this.enregistreur));
		this.commandes.put(REDO_CMD, new Refaire(this.moteur, this, this.enregistreur));
	}
	...
	private void displayHelp() {
		...
		// If there are some states to go back to
		if (this.moteur.canUndo()) {
			System.out.println("	" + UNDO_CMD + "  -> Defaire");
		}

		// If there are some states to go in the future
		if (this.moteur.canRedo()) {
			System.out.println("	" + REDO_CMD + "  -> Refaire");
		}
			...
			try {
				...
				switch (input) {
					...
					case UNDO_CMD:
						...
						// Only if not empty
						if (this.moteur.canUndo()) {
							((Defaire)this.commandes.get(UNDO_CMD)).getValues();
							this.commandes.get(UNDO_CMD).execute();
						}
						else {
							System.out.println("Vous devez avoir effectué des actions avant de pouvoir les défaire.");
							System.out.println("Veuillez sélectionner une autre commande.");
						}
						break;
					...
					case REDO_CMD:
						...
						// Only if not empty and the index is not the last element
						if (this.moteur.canRedo()) {
							((Refaire)this.commandes.get(REDO_CMD)).getValues();
							this.commandes.get(REDO_CMD).execute();
						}
						else {
							System.out.println("Vous devez avoir défait des actions avant de pouvoir les refaire.");
							System.out.println("Veuillez sélectionner une autre commande.");
						}
						break;		
